...
export class ArgumentMapper {
  public static resolveDynamicMapping(argumentObject: any, sourceUseCaseUow: object, forceClone: boolean, commandArgs?: object): any {
      if(!argumentObject ){
          return {};
      }
      ...
      // return settingsGetter.pipe(map((allSettingsObjects)=>{
          ...
          let mappingEntries = (argumentObject['mapDynamic'] as {use: string, for:string}[]);
          ...
          for(var mappingEntry of mappingEntries) {
              ...
              let sourcePath: string = mappingEntry.use.substring(mappingEntry.use.indexOf("://") + 3);
              ...
              if(error){
                  ...
                  console.debug("resolveArgumentMapping: using '" + value + "' for '" + mappingEntry.for + "'");
  ...
  private static applyValue(value: any, targetObject: any, targetPath: string){
      ...
      if(idx < 0){
          ...
          if(propName){
              ...
              ArgumentMapper.applyValue(value, targetObject[propName],targetPath.substring(idx+1));
          }
      }
  }
